...
public class SerializingTranscoder extends BaseSerializingTranscoder implements CacheTranscoder {
	 ...
	 */
	public final Object decode(CachedObject d) {
		byte[] data = d.getData();
	...
	protected final Object decode0(CachedObject cachedData, byte[] data, int flags) {
		Object rv = null;
		if ((cachedData.getFlags() & SERIALIZED) != 0 && data != null) {
			rv = deserialize(data);
		} else {
	 ...
	 */
	public final CachedObject encode(Object o) {
		byte[] b = null;
		int flags = 0;
		...
		if (b.length > this.compressionThreshold) {
			...
			if (compressed.length < b.length) {
				...
				if (log.isDebugEnabled()) {
					log.debug("Compression increased the size of " + o.getClass().getName()
							+ " from " + b.length + " to " + compressed.length);
				}
			}
		}
		return new CachedObjectImpl(flags, b);
	}
}
